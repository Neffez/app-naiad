...
- fix: only send message if there is a run tomorrow
...
- fix: resume via start (no dialog); idempotent stop/cancel + discard_snapshot
- fix(sequences): real resume (no start dialog) + idempotent stop/cancel
...
- feat(ui): in-app toast notifications for action results
- fix(sequences): reject empty-zone start; log start path; quiet apscheduler
- feat(config): searchable picker for notify targets
- feat(config): searchable HA entity picker with type hint
- fix(ui): mobile config scroll, theme in settings, slider, schedule button, emergency stop; dynamic version; addon version automation
- feat(config): enter a name for zones/sequences, generate the id internally
- fix: Dockerfile builds the image (editable install ordering + real frontend)
- feat: ingress-trust auth mode, coexisting with password (Phase 6d)
- feat: detect HA add-on context and use Supervisor proxy connection
- fix: apply ruff format to test_runtime_reload and test_tracking
- fix: prevent password-auth lockout and harden config-reload edges
- feat: UI-based configuration and ingress-ready frontend (Phase 6a/6b)
- test: add LiterTracker tests and expand Phase 5 docs
- fix: address Low findings (L-1, L-5, L-8..L-12)
- feat: complete Phase 4 — full frontend design implementation
- feat: Phase 4 improve frontend
- feat: Phase 4 — APScheduler, WebSocket, React frontend
- feat: Phase 3 — REST API layer
- feat: Phase 2 — driver protocols, sequence runner, factor calculation
- feat: Phase 1 — backend skeleton, config schema, HA client
- feat: Phase 0 — repo infrastructure and CI/CD    - CLAUDE.md: code rules (English, architecture reference, test requirements)    - GitHub Actions: CI (ruff, mypy, pytest) and release (multi-arch GHCR push on tag)    - Dockerfile: multi-stage build (frontend placeholder + python:3.12-slim runtime)    - docker-compose.yml, config.example.yaml, .env.example    - pyproject.toml: project metadata, ruff/mypy/pytest config, dev dependencies    - .pre-commit-config.yaml: ruff + mypy hooks    - .gitignore: Python, Node, app data, IDE    - design/: Claude Design prototype files (visual reference)    - docs/openapi.yaml: complete REST API contract    - README: status update, hardware disclaimer, quickstart section
- initial commit
